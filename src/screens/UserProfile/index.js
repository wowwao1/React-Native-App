...
import { View, Text ,Image,Keyboard,StyleSheet,TouchableOpacity,FlatList,ScrollView,Alert,Modal} from 'react-native';
...
getUserPost = (userid) => {
  ...
  this.setState({ userposts: data.data })
...
sendFriendRequestNotification = async(user) => {
  let authUser = await getData('user');
  let data = new FormData();
  data.append("action", "sendNotification");
  data.append("user_id", JSON.parse(authUser).id);
...
sendFollowNotification = async(user) => {
  let authUser = await getData('user');
  let data = new FormData();
  data.append("action", "sendNotification");
  data.append("user_id", JSON.parse(authUser).id);
