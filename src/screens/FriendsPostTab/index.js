...
import { View, Text, Image, Keyboard, TouchableOpacity, FlatList, ScrollView, Modal, Alert, ActivityIndicator } from 'react-native';
...
class FriendsPostTab extends Component {
	...
	onRefresh = async () => {
		let user = await getData("user");
		user = JSON.parse(user);
		this.setState({ isFetching: true }, function () { this.getFriendPosts(user.id) });
	}
	convertUnicode = (input) => {


		input.replace("U+1F602", "\uD83D\uDE02");
	...
	blockUser = async () => {
		...
		sendRequest("POST", data).then(data => {
			if (data.status) {
				...
				this.getFriendPosts(JSON.parse(authUser).id)
			}
		})
	}
	_renderFooter = () => {
